...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
  ...
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
        /*if(image[i] = 0xFFFF){
            ST7735_DrawPixel(x, y-h+1, image[i]); //i have zero idea wtf is happening
            i = i + 1;
        }else{*/
            // send the top 8 bits
...
void ST7735_DrawBitmapv2(int16_t x, int16_t y, const uint16_t *image1, const uint16_t *image2, int16_t w, int16_t h){
  int16_t skipC = 0;                      // non-zero if columns need to be skipped due to clipping
  int16_t originalWidth = w;              // save this value; even if not all columns fit on the screen, the image is still this width in ROM
  int i = w*(h - 1);
  ...
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
        if(image1[i] == 0xFFFF){
            SPI_OutData((uint8_t)(image2[i] >>8));
            SPI_OutData((uint8_t)image2[i]);
            i = i + 1;
        }else{
            // send the top 8 bits
            SPI_OutData((uint8_t)(image1[i] >> 8));
            // send the bottom 8 bits
            SPI_OutData((uint8_t)image1[i]);
            i = i + 1;                        // go to the next pixel
        }
    }
    i = i + skipC;
...
static uint16_t buf[s_width*s_height]; //size of sprite

void ST7735_CopyImage(uint16_t x, uint16_t y, const uint16_t *background){
    int image_i = 128*(159 - y) + x;
    int buffer_i = 0;
    for (int i = 0; i < s_height; i++){ //height
        for (int j=0; j< s_width; j++){ //width
            buf[buffer_i++] = background[image_i++];
        }
        image_i += 128 - s_width; //128-width
    }
}

void ST7735_Merge(const uint16_t *sprite, uint32_t mask){
...
void ST7735_DrawNew(uint16_t x, uint16_t y){
    ST7735_DrawBitmap(x, y, buf, s_width, s_height);
}

void ST7735_DrawSprite(uint16_t x, uint16_t y, const uint16_t *background, const uint16_t *sprite, uint32_t mask){
    ST7735_CopyImage(x, y, background);
    ST7735_Merge(sprite, mask);
    ST7735_DrawNew(x, y);
}

void ST7735_EraseSprite(uint16_t x, uint16_t y, const uint16_t *background) {
    ST7735_CopyImage(x, y, background);
    ST7735_DrawNew(x, y);
}

void ST7735_DrawAndReplace(int16_t x, int16_t y, const uint16_t *background_image, const uint16_t *new_image, int16_t w, int16_t h){
    int i = w*(h - 1); //this seems wrong :D
}
//------------ST7735_DrawCharS------------
